...
class PeruttLoader(object):
    ...
    def __init__(self,
                 ...
                 ref_scp=None,
                 sample_rate=16000):
        self.mix = WaveReader(mix_scp, sample_rate=sample_rate) # self.mix只是一个WaveReader的实例，具有所有WaveReader的属性
    ...
    def __iter__(self):
        ...
        for key, mix in self.mix:  # 这句话启动了WaveReader的iterator
...
class ChunkSplitter(object):
    ...
    def split(self, eg):
        ...
        if N < self.chunk_size:
            ...
            s = random.randint(0, N % self.least) if self.train else 0  # if in train=False, i.e., in test set, then the start point is 0. Still in test set, the wav will be splitted as small trunks. Actually, each sentence in a batch is not the original sentence, but the splitted small trunks. In this way, even in test set, a sentence could be processed at the same batch.  
            while True:
    ...
    def _fetch_batch(self):
        while True:
            if len(self.load_list) >= self.cache_size:
                break
            try:
                eg = next(self.load_iter) # load a sentence, eg is a dict.
                cs = self.splitter.split(eg) # cut or pad to a chunk size sentence, cs is a list
                self.load_list.extend(cs) # load_list 是cs中的dict组成的大的list
        ...
        for s in range(0, N - self.batch_size + 1, self.batch_size):
            batch = default_collate(self.load_list[s:s + self.batch_size]) # default_collate将数据读成tensor
            blist.append(batch)  # blist中是许多小batch，而load_list中则是一句话
        # update load_list
